...
int main(const int argc, const char** argv) {
    ...
    const size_t bytes = nParticles * sizeof(float);
    ...
    initialize_states(h_x, nParticles, -50, 50);
    initialize_states(h_y, nParticles, -50, 50);
    initialize_states(h_vx, nParticles, -2, 2);
    initialize_states(h_vy, nParticles, -2, 2);
    ...
    fwrite(&nParticles, sizeof(int), 1, f);
    ...
    for (float t = 0.0f; t < T; t += dt) {
        ...
        size_t wrote;
        wrote = fwrite(h_x, sizeof(float), nParticles, f);
        if (wrote != (size_t)nParticles) {
            perror("fwrite x");
            return 1;
        }
        wrote = fwrite(h_y, sizeof(float), nParticles, f);
        if (wrote != (size_t)nParticles) {
            perror("fwrite y");
            return 1;
        }
